...
declaw's `hwbp` mode is that idea wired up. It arms the `perf_event_open` breakpoint on `ssl_log_secret` in every thread of the target, rescans as new threads spawn, and on each hit reads the secret and `client_random` back out of the register file and `/proc/pid/mem`. Those are the exact bytes the library would have written to `SSLKEYLOGFILE`, assembled into standard NSS key-log lines that decrypt the capture in Wireshark.
...
## getting an agent past PairIP's guard dog

The two rungs above win by putting nothing in the process. There is a third case that does the opposite on purpose. cronet's keys come out through friTap, and friTap injects: it spawns under Frida and sets inline hooks on the app's own BoringSSL. Under a PairIP-hardened app that is exactly the thing that dies at spawn. So before friTap reads a single key, something has to keep PairIP's guard dog from noticing it.

PairIP's antidebugger, reverse-engineered from a real `libpairipcore.so` and a published decompile of its `antidebugger()`, is a handful of checks that all funnel through one place. It calls `ptrace` on itself to see if anyone is already attached. It `fork()`s a child whose only job is to `ptrace`-scan the parent. It walks `/proc` and `dl_iterate_phdr` for `frida`, a gadget, a nonzero `TracerPid`. And it calls `abort()` the instant any of that trips. The useful part is that none of it goes through named wrappers. It calls libc `syscall()` directly, so one hook covers the lot.

declaw's `anti_pairip.js`, loaded into friTap at spawn, sits on that choke point and answers each check with the boring correct lie.

![how declaw's anti_pairip hook neuters PairIP's antidebugger at the libc choke point, and the wall it does not touch]({{ '/assets/img/posts/declaw-pairip.png' | relative_url }})

`syscall(ptrace, ...)` returns `EPERM`, so the self-check sees no tracer. `clone()` with `flags == 0` is a real `fork()`, and the child that comes back gets `SIGKILL` before it scans anything. The `/proc` reads get their frida lines filtered and `TracerPid` rewritten to `0`, and the `dl_iterate_phdr` walk never sees the gadget's module. `abort` and `exit`, scoped to callers whose return address is inside `libpairipcore`, get swallowed. The agent spawns, attaches, and stays alive.

There is a matching move on the other side of the same scan. The frida-server binary declaw pushes is renamed on the device, and optionally its `frida` / `GumScript` symbols, strings, and thread names are stripped out of the binary itself, so a packer that scans process names and loaded strings for `frida` finds nothing. That rewrite is off by default, because on some Frida versions it corrupts the agent and crashes the target, so it is there for the specific case where server detection is the thing blocking you and nothing else is.

Now the honest part, because this is an arms race and the tool says so itself. This defeats the antidebugger. It does not touch PairIP's other wall, the in-memory hash of its own `.text`. friTap's inline hooks change bytes in the process, and a build that hashes itself still catches them, `anti_pairip.js` or not. So this gets an agent past the guard dog, not past the fence, and on the hardest builds the answer is still the two rungs that hook nothing. It is grounded in reverse engineering and it is generic, which is another way of saying some hardened apps resist it. One more thing I will be blunt about: it was a silent no-op on the Frida 17.x this tool actually ships until I caught that in an audit and fixed it, so what I can vouch for is the mechanism, not a fresh end-to-end run against a current PairIP build.
...
- the Frida gadget dropped into every ABI, with the public [httptoolkit/frida-interception-and-unpinning](https://github.com/httptoolkit/frida-interception-and-unpinning) bundle beside it, loaded from the Application class's smali at process start. Android 15+ refuses to map a `.so` whose LOAD segments are 4 KB-aligned, and the gadget ships 4 KB-aligned, so declaw re-aligns the injected gadget to 16 KB first or it SIGSEGVs on a Pixel 8,
...
- the OkHttp `CertificatePinner.check()` method patched to a no-op straight in smali, and the same no-op treatment for TrustKit's `PinningTrustManager` and appmattus's `CertificateTransparencyTrustManager.checkServerTrusted`, the Java-layer pins that mempatch deliberately cannot reach, plus the httptoolkit root-detection disable hook,
- a static byte-patch of `libflutter.so`, or reFlutter's pre-patched engine swapped in when the engine's snapshot hash is one it publishes, for Flutter apps, so the bypass survives even where the gadget cannot load,
- a static byte-patch of a bundled non-Flutter BoringSSL (`libttboringssl.so` and friends), the on-disk counterpart to mempatch's live flip,
...
![what declaw injects and patches into an APK on the repackage path, from the manifest down to the native libraries]({{ '/assets/img/posts/declaw-repack.png' | relative_url }})

The Flutter static patch is the rung I lean on hardest, because the gadget is the fragile part of this whole path. The same Frida Gum crash that killed injection on the hardened app also makes the gadget unreliable on newer devices, and a Flutter app ignores your system proxy on top of that, so you can have a gadget that loads and still see no traffic. Patching `ssl_verify_peer_cert` inside `libflutter.so` on disk sidesteps both problems at once: no agent to crash, and the `connect()` redirect drags the socket to your proxy regardless. That combination is what got a real Flutter banking app decrypting cleanly into my proxy end to end, on a build where the gadget alone had gotten me nothing.

There is a second Flutter layer under that byte-patch, and it is less hand-rolled. reFlutter publishes whole pre-patched Flutter engines, keyed by the engine's snapshot hash. So declaw reads the app's `libflutter.so`, finds the snapshot hash sitting in its bytes, and if that hash is one reFlutter knows, it downloads the matching pre-patched engine for that exact version and ABI and swaps the whole `.so` in, no byte-surgery at all. When the hash is one it does not publish, declaw falls back to its own byte-patch or to the Frida hooks.

![reFlutter path: match libflutter's engine snapshot hash, swap in the pre-patched engine, or fall back to the byte-patch]({{ '/assets/img/posts/declaw-reflutter.png' | relative_url }})

Two of those patches close gaps the in-memory rungs leave. The OkHttp `CertificatePinner.check()` no-op reaches the Java-layer pin mempatch cannot: the smali patch removes the pin at its source, before the handshake runs, which is why an OkHttp-pinned app takes this route instead of the eight-byte flip. And the static BoringSSL patch is the on-disk version of mempatch for a device with no root: overwrite `ssl_verify_peer_cert`'s prologue in a bundled `libssl.so` so it returns `ssl_verify_ok`, ship the patched `.so` in the repackaged APK, and there is no frida and no live process to catch. I want to be exact about how far that last one is proven, because it is the least battle-tested thing in the tool. The eight-byte stub is byte-exact and unit-tested, and against a from-source BoringSSL in the lab it made a verifying client accept a self-signed cert with a clean negative control. What it has not cleared here is the full loop: a real Android repackage, re-sign, and run against a live pinned app, plus locating the offset in a stripped release `.so`. So it is a proven primitive on a known BoringSSL, not yet a proven end-to-end bypass on an arbitrary app.

![the same ssl_verify_peer_cert flipped two ways: static on disk in the APK without root, or live through /proc/pid/mem with root]({{ '/assets/img/posts/declaw-two-flips.png' | relative_url }})
...
- **The hardest integrity checks.** The anti-PairIP hook gets an agent past the antidebugger, but PairIP's in-memory `.text` hash is a separate wall it does not touch: that hash still catches friTap's inline hooks, so against the most hardened apps a capture comes back with no keys. That is exactly the case hwbp and mempatch exist for, because neither places a hook, but it is per-build reverse engineering, not a button.
