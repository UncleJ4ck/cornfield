...
- the OkHttp `CertificatePinner.check()` method patched to a no-op straight in smali, which is the Java-layer pin that mempatch deliberately cannot reach, plus the httptoolkit root-detection disable hook,
