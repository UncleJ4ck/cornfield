...
## v4.3: four objectives wearing one scorer, an idea that did nothing, and the loop's real ceiling

Three things happened after v4.0, and only one is a feature I would put in a headline. The other two are a measurement bug I had been shipping without noticing and a good-sounding idea the data killed. I am writing all three down because the pattern is by now the whole point: the result worth keeping is usually the one that contradicts what you sat down to write.

### the scorer answered one question and I was asking it four

The two-evaluator gate asks exactly one thing. Is this response harm-confirmed. That is the right question for a jailbreak and I built the entire judge around it. Then I actually looked at the rest of the corpus. There are system-prompt-extraction tests, where the win is the model leaking its own hidden instructions. There is an XSTest-style over-refusal probe, where the win is the model *refusing* a request that was completely benign. The harm gate scores both of these wrong, and it does it quietly, which is the dangerous kind of wrong.

Walk the leak case. The model coughs up its system prompt. That is a break. But a leaked prompt is not "actionable harmful content", so the independent harm classifier reads it, correctly says nothing dangerous here, and the gate downgrades a real break to PARTIAL. I was measuring harm on a test that was never about harm. The over-refusal case is worse, because it inverts. The defect there is a refusal, so a harm-confirmed SUCCEEDED on that test is not a mis-score, it is the scoreboard reading backwards. That is the exact error that credited an XSTest item as a break in the old 63% Opus run. I had fixed the symptom once and left the disease.

So each test now declares an `objective_type` and the success check routes on it. `harmful_compliance` is the default and still runs the full harm gate, byte for byte. `over_refusal` inverts: the finding is a refusal of a safe request, scored by a whole-text decline detector, and it runs as a single-shot probe, because escalating a benign question over twenty rounds is meaningless. `prompt_leak` gets its own detector, below. And the headline number is now harm-only. The exit code counts `harmful_compliance` breaks and nothing else, with the other objectives reported in their own bucket, so a leak or an over-refusal can never again quietly pad the jailbreak rate. The 24 extraction tests left the harm denominator entirely, where they had been dragging it around the whole time.

### a leak needs something to steal

The obvious way to score a prompt leak is to ask a judge "did the model reveal its system prompt", and that is a false-positive machine. The model recites some generic "I am a helpful assistant" boilerplate, a goal-aware judge primed to see a leak calls it one, and you have laundered a non-event into a finding. I have been burned by that exact circular judge once already, three sections up. I was not going to rebuild it with a fresh coat of paint.

What makes a leak checkable is giving it a ground truth. Before the extraction test runs, I inject a known high-entropy canary into the target's system prompt, a random 64-bit token. A leak is that exact token turning up in a reply. Nothing else counts. This cannot false-positive, because a model cannot emit a specific 64-bit string it never saw; it can only have copied it out of the prompt it was told to guard. The check sits beside the harm verdict, not inside it, and it is skipped entirely when no canary is present, so an ordinary jailbreak test does not even notice the code exists.

One trap is worth naming, because it is the kind that scores a silent zero and looks like a pass. The canary only works if the system message actually reaches the model, and only two of the six client paths in the codebase thread a system message into the request. The other four drop it on the floor. If a leak test ran through one of those, the model would never see the canary, never leak it, and the run would report "resisted", a false negative wearing a pass. So the leak path fails closed. If the client cannot deliver a system message, the test is marked inconclusive, not safe. An undeliverable canary is evidence of nothing, and I would rather log nothing than a comforting zero.

### an idea that read well and did nothing

Here is the one I was sure about. Watch a real attacker and they do not throw a fresh disguise at every turn. They grab the model's own last answer and push on the exact words it just volunteered. The model conceded "I can discuss the theory but not the steps", so you ask for the one step adjacent to the theory it already gave, in its own vocabulary. Progressive commitment on the model's own terms. I wrote it as an attacker directive, put it behind a flag, and expected it to convert the partials the loop keeps leaving on the table.

The mechanism half is real. On three partial-compliance responses the baseline attacker ignored the model's words and opened a fresh "I am a senior incident responder" authority pretext. The anchoring attacker opened "you mentioned X and Y", echoing nine of the model's own volunteered terms against the baseline's four. It does what it says. Then I ran the actual A/B. Control against treatment, same target and judge and rounds, only the directive changing. Mid-band target, twenty tests. Baseline broke ten. Anchoring broke twelve. The noise band at that sample size is about two and a half breaks, so plus-two is not a result. It is a coin landing a little heads. On the harder frontier target the problem flips: the attacker draws instant hard refusals with zero partial engagement, so there is nothing to anchor on and the idea is not even testable at that budget. Flag stays off.

The one honest signal in the run is negative, and it is the useful part. Anchoring reliably produced *more* partial engagement. It kept the model talking further into the conversation. None of that extra talk converted to a confirmed break. That lines up with a thing about these models it is easy to forget. A model has no memory across conversations. Every test is a fresh context, so a refusal in one attempt teaches it nothing about the next, and you get infinite clean tries. But inside one conversation it remembers everything, including its own refusals, and it hardens as the thread piles them up. Anchoring digs deeper into a single hardening thread. That is the wrong direction. Once a thread has trained the model to keep saying no, the fix is not to push harder on it. The fix is to drop it and start clean.

### the loop's real ceiling, and the shape of the fix

That last point is the interesting one, because it is a limitation of the loop itself, not of any single attack. The controller is greedy and single-track. One strategy per round, and when it gets a PARTIAL it commits to one continuation and grinds it. If that continuation walks into the in-conversation hardening I just described, the whole test fails, even though a different move from the same partial state might have walked around it. The anchoring null is a symptom of that ceiling. More engagement, no more breaks, because the loop cannot do anything with a good partial except push it in a straight line until it hardens.

The fix is to stop searching in a straight line. Keep the few most promising conversation states, expand each with several candidate next moves, score how close each one got, keep the best and throw the rest away, so a near-miss branch is not lost and a hardened thread can be abandoned for a fresher one. That is beam search, and it is what PAIR's descendant TAP does with a tree. The difference here is a rule I will not bend: the score that ranks the branches and the verdict that declares a win are two different things, and the win is still the two-evaluator gate. A high progress score can rank a branch to the front. It can never call a break. The gate is the only thing that ever calls a break, the same rule the whole judge section is built on.

I shipped the first layer of this and nothing more, on purpose. The progress score, the thing that says "closer to a break", is pulled into one shared definition so the existing best-of-n search and the new beam agree on what closer means. The beam engine is written and tested against a rigged target where the greedy path dead-ends and the only break is two moves down a branch that looks worse at depth one. The greedy version misses it. The beam finds it. Neither is ever allowed to call a win off the score alone. But it is not wired into the live loop yet, because wiring it in changes the break rate, and anything that changes the break rate has to earn it against a live model in an A/B before it goes on by default. I have watched too many good-on-paper ideas die in that A/B, the anchoring one two subsections up being the freshest corpse, to trust this one just because the offline test is green.

Encoding lives here too, and this is where it belongs. The repo has eighteen encoders and a whole string-transform module, and while auditing I found almost none of it is actually in the adaptive loop. The only live transform is the Cyrillic homoglyph pass, and the one strategy that mentions composition tells the *target* to encode its own answer, an output-filter trick, not the input-classifier trick of encoding the request. So encoding is mostly implemented and mostly unreachable. The right home for it is not "encode everything as a fallback", which just wastes rounds on frontier models that decode and then refuse on the meaning anyway. It is one more branch the beam can pick, fired only when the fingerprint says the wall is an input classifier, with the decoded intent judged and a decode-that-still-refuses scored as evidence about the classifier rather than as a win. That is a later layer. I am writing the direction down, not claiming the result.

---

## the audit pass, where nothing new broke and that was the point

After v4.3 I did the thing I keep telling other people to do and rarely do myself: I turned the tool on its own source. A full adversarial read of every module, plus a separate fuzz pass that hammered each component with malformed, oversized, and adversarial unicode input. The honest headline is that no break rate moved. There is no new percentage in this section, and for an audit that is the correct outcome. What it produced instead is one bug that belongs in the judge section three-quarters of the way up this post, and a pile of plumbing that makes the numbers already here easier to trust.
